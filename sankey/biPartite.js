!function(){
	var bP={};	
	var b=30, bb=150, height=600, buffMargin=1, minHeight=2.5;
	...
	function drawPart(data, id, p){
		...
		//d3.select("#"+id).select(".part"+p).append("g").attr("fill",function(d){ return colors[d.key1];});
	...
	function drawEdges(data, id){
		...
		d3.select("#"+id).select(".edges").selectAll(".edge")
			...
			.attr("points", edgePolygon).style("fill","lightgray")
				...
				d3.select("#"+biP.id)
					...
					.on("mouseover", function(d, i) { 
						bP.selectSegment(data, p, i) 
						 d3.select(this)[0][0]
						//   .attr("fill",function(d){ return colors[d.key1];});
					})
					// .on("mouseover", function(d, i) {
					// 	return d3.select("#"+id).select(".part"+p).select(".subbars")
			    //   .style("fill",function(d){ return colors[d.key1];});
					// })
//here
					//.on("mouseover",function(d, i){ return colors[d.key1] });
	...
	bP.selectSegment = function(data, m, s){
		...
		data.forEach(function(k){
			...
			selectedBar.select(".mainrect").style("stroke-opacity",1).attr("fill","darkgreen");
			// .style("fill", function(d){ 
			// 	console.log("d", d) 
			// 	return colors[d.key1];
			// });		
			selectedBar.select(".barlabel").style('font-weight','bold').attr("fill","darkgreen");
			selectedBar.select(".barvalue").style('font-weight','bold').attr("fill","darkgreen");
	...
	bP.deSelectSegment = function(data, m, s){
		data.forEach(function(k){
			...
			selectedBar.select(".mainrect").style("stroke-opacity",0).attr("fill","lightgray")			
			selectedBar.select(".barlabel").style('font-weight','normal').attr("fill","black")
			selectedBar.select(".barvalue").style('font-weight','normal').attr("fill","black")
